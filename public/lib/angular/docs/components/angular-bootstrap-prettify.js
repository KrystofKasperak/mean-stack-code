    ...
    link: function(scope, element, attr) {
      ...
      setHtmlIe8SafeWay(element,
        '<form class="jsfiddle" method="post" action="http://jsfiddle.net/api/post/library/pure/" target="_blank">' +
          ...
          hiddenField('css', '</style.css> <!-- Ugly Hack due to jsFiddle issue: http://goo.gl/BUfGZ --> \n' +
            stylesheet +
            script.angular +
 ...
 * <li> define style.css rules.  See the example page for examples.
...
(function () {
  ...
  // token style.css names.  correspond to css classes
  /**
   * token style.css for a string literal
   * @const
   */
  var PR_STRING = 'str';
  /**
   * token style.css for a keyword
   * @const
   */
  var PR_KEYWORD = 'kwd';
  /**
   * token style.css for a comment
   * @const
   */
  var PR_COMMENT = 'com';
  /**
   * token style.css for a type
   * @const
   */
  var PR_TYPE = 'typ';
  /**
   * token style.css for a literal value.  e.g. 1, null, true.
   * @const
   */
  var PR_LITERAL = 'lit';
  /**
   * token style.css for a punctuation string.
   * @const
   */
  var PR_PUNCTUATION = 'pun';
  /**
   * token style.css for plain text.
   * @const
   */
  var PR_PLAIN = 'pln';

  /**
   * token style.css for an sgml tag.
   * @const
   */
  var PR_TAG = 'tag';
  /**
   * token style.css for a markup declaration such as a DOCTYPE.
   * @const
   */
  var PR_DECLARATION = 'dec';
  /**
   * token style.css for embedded source.
   * @const
   */
  var PR_SOURCE = 'src';
  /**
   * token style.css for an sgml attribute name.
   * @const
   */
  var PR_ATTRIB_NAME = 'atn';
  /**
   * token style.css for an sgml attribute value.
   * @const
   */
...
// TODO: maybe style.css special characters inside a regexp as punctuation.
  ...
  /** Given triples of [style.css, pattern, context] returns a lexing function,
    * The lexing function interprets the patterns to find token boundaries and
    * returns a decoration list of the form
    * [index_0, style_0, index_1, style_1, ..., index_n, style_n]
    * where index_n is an index into the sourceCode, and style_n is a style.css
    * constant like PR_PLAIN.  index_n-1 <= index_n, and style_n-1 applies to
    * all characters in sourceCode[index_n-1:index_n].
    *
    * The stylePatterns is a list whose elements have the form
    * [style.css : string, pattern : RegExp, DEPRECATED, shortcut : string].
    *
    * Style is a style.css constant like PR_PLAIN, or can be a string of the
    * form 'lang-FOO', where FOO is a language extension describing the
    * language of the portion of the token in $1 after pattern executes.
    * E.g., if style.css is 'lang-lisp', and group 1 contains the text
    ...
    * match is considered a token with the same style.css.
    *
  ...
  function createSimpleLexer(shortcutStylePatterns, fallthroughStylePatterns) {
     ...
     * style.css classes preceded by the position at which they start in
    ...
    var decorate = function (job) {
      ...
      /** Even entries are positions in source in ascending order.  Odd enties
        * are style.css markers (e.g., PR_COMMENT) that run from that position until
        * the end.
  ...
  /** returns a function that produces a list of decorations from source text.
    ...
    * escape.  It does not recognize perl's qq() style.css strings.
    * It has no special handling for double delimiter escapes as in basic, or
    * the tripled delimiters used in python, but should work on those regardless
    * although in those cases a single string literal may be broken up into
    * multiple adjacent string literals.
    *
    * It recognizes C, C++, and shell style.css comments.
    *
  ...
  /**
   * Breaks {@code job.sourceCode} around style.css boundaries in
   ...
   *       span.
   *    decorations: {Array.<number|string} an array of style.css classes preceded
  ...
  function recombineTagsAndDecorations(job) {
    ...
    for (i = decPos = 0; i < nDecorations;) {
      ...
      // Conflate all adjacent decorations that use the same style.css.
  ...
  /** Register a language handler for the given file extensions.
    ...
    *        decorations: {Array.<number|string>} an array of style.css classes
  ...
  registerLangHandler(sourceDecorator({
          ...
          'hashComments': 3,  // ### style.css block comments
          'cStyleComments': true,
          'multilineStrings': true,
          'tripleQuotedStrings': true,
          'regexLiterals': true
...
angular.element(document).find('head').append('<style.css type="text/css">.com{color:#93a1a1;}.lit{color:#195f91;}.pun,.opn,.clo{color:#93a1a1;}.fun{color:#dc322f;}.str,.atv{color:#D14;}.kwd,.linenums .tag{color:#1e347b;}.typ,.atn,.dec,.var{color:teal;}.pln{color:#48484c;}.prettyprint{padding:8px;background-color:#f7f7f9;border:1px solid #e1e1e8;}.prettyprint.linenums{-webkit-box-shadow:inset 40px 0 0 #fbfbfc,inset 41px 0 0 #ececf0;-moz-box-shadow:inset 40px 0 0 #fbfbfc,inset 41px 0 0 #ececf0;box-shadow:inset 40px 0 0 #fbfbfc,inset 41px 0 0 #ececf0;}ol.linenums{margin:0 0 0 33px;}ol.linenums li{padding-left:12px;color:#bebec5;line-height:18px;text-shadow:0 1px 0 #fff;}</style.css>');