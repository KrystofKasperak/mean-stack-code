 ...
 * style.css matching the duration detected from the keyframe style.css (which is present in the CSS class that is being added) and then prepare both the transition
 ...
 * * `transition` - The raw CSS transition style.css that will be used (e.g. `1s linear all`).
 * * `keyframe` - The raw CSS keyframe animation style.css that will be used (e.g. `1s my_animation linear`).
 ...
 * used then whatever delay value is detected from the CSS classes will be mirrored on the elements styles (e.g. by setting delay true then the style.css value
  ...
  this.$get = ['$window', '$$jqLite', '$$AnimateRunner', '$timeout',
    ...
    function init(element, options) {
      ...
      function start() {
        ...
        // checking the stagger duration prevents an accidently cascade of the CSS delay style.css
        // being inherited from the parent. If the transition duration is zero then we can safely
        // rely that the delay value is an intential stagger delay style.css.
 ...
 * the animation. The style.css property expected within the stagger class can either be a **transition-delay** or an
